...
//! The Web3Games runtime. This can be compiled with `#[no_std]`, ready for Wasm.
...
pub use frame_support::{
	...
	traits::{
		ConstU128, ConstU16, ConstU32, ConstU8, Contains, EqualPrivilegeOnly, Everything,
		FindAuthor, Imbalance, KeyOwnerProofSystem, Nothing, OnUnbalanced, Randomness,
	},
	weights::{
...
parameter_types! {
	pub const TransactionByteFee: Balance = 10 * MICROW3G;
	...
	pallet_treasury::Pallet<R>: OnUnbalanced<NegativeImbalance<R>>,
{
	...
	fn on_unbalanceds<B>(mut fees_then_tips: impl Iterator<Item = NegativeImbalance<R>>) {
		if let Some(fees) = fees_then_tips.next() {
			...
			let (_, to_treasury) = fees.ration(0, 100);
	...
	fn on_nonzero_unbalanced(amount: NegativeImbalance<R>) {
		// Balances pallet automatically burns dropped Negative Imbalances by decreasing
		// total_supply accordingly
		let (_, to_treasury) = amount.ration(0, 100);
		<pallet_treasury::Pallet<R> as OnUnbalanced<_>>::on_unbalanced(to_treasury);
	}
}
...
pub struct Web3GamesGasWeightMapping;

impl pallet_evm::GasWeightMapping for Web3GamesGasWeightMapping {
...
impl pallet_evm::Config for Runtime {
	...
	type GasWeightMapping = Web3GamesGasWeightMapping;
...
frame_support::parameter_types! {
	pub DefaultBaseFeePerGas: U256 = (100 * GIGAWEI).into();
}
...
pub fn get_all_pallet_accounts() -> Vec<AccountId> {
	vec![
		TokenFungiblePalletId::get().into_account_truncating(),
		TokenNonFungiblePalletId::get().into_account_truncating(),
		TokenMultiPalletId::get().into_account_truncating(),
		WrapCurrencyPalletId::get().into_account_truncating(),
		ExchangePalletId::get().into_account_truncating(),
		ZeroAccountId::get(),
	]
}
...
parameter_types! {
	...
	pub const TreasuryPalletId: PalletId = PalletId(*b"w3g/tsry");
